...
export const store = new Vuex.Store({
	...
	mutations: {
		...
		checkOffListItem: (state, payload) => {
			...
			state.lists[(payload.listID)].listItems[payload.listItemIndex].checked = !state.lists[(payload.listID)].listItems[payload.listItemIndex].checked;
		},
		...
		addMeal: (state, payload) => {
			...
			payload.recipe.ingredients.forEach(ingredient => {
				...
				if ( state.lists[state.lists.length - 1].listItems.some(function(element, index, array){ return element.name === ingredient.name })) {
					console.log('item already in there!');
					// If it is already in list, add to the quantity
					var item = state.lists[state.lists.length - 1].listItems.find(element => element.name === ingredient.name)
